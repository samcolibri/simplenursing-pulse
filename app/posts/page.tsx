...
const PLATFORM_COLORS: Record<string, string> = { tiktok: '#75c7e6', instagram: '#fc3467', facebook: '#00709c' }
    ...
    <div className="p-6 max-w-screen-xl mx-auto space-y-5">
      ...
      <div className="flex items-start gap-3 bg-[#0f110a] border border-[#fad74f]/20 rounded-xl p-4">
        ...
        <div className="text-sm">
          ...
          <span className="text-gray-400"> to see your own Instagram + Facebook post-level insights (views, reach, saves per post). Currently showing Apify-scraped + sample data.</span>
        </div>
      </div>
      ...
      <div className="flex gap-3 flex-wrap">
        {['all', 'tiktok', 'instagram', 'facebook'].map(p => (
          <button key={p} onClick={() => setPlatform(p)} className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${platform === p ? 'bg-[#75c7e6]/10 border border-[#75c7e6]/30 text-[#75c7e6]' : 'bg-[#0d1117] border border-[#1e2433] text-gray-400 hover:text-white'}`}>{p}</button>
        ))}
        <select value={sort} onChange={e => setSort(e.target.value)} className="ml-auto px-3 py-1.5 rounded-lg text-xs bg-[#0d1117] border border-[#1e2433] text-gray-400 focus:outline-none focus:border-[#75c7e6]/30">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#1e2433]">
                {['Platform', 'Date', 'Caption', 'Views', 'Likes', 'Comments', 'Shares', 'ER%', 'Topic'].map(h => (
            ...
            <tbody>
              {filtered.map((post, i) => (
                <>
                  <tr key={i} onClick={() => setExpanded(expanded === i ? null : i)} className={`border-b border-[#1e2433]/50 hover:bg-[#161b22] cursor-pointer transition-colors ${post.isComp ? 'border-l-2 border-l-[#fad74f]' : ''}`}>
                    <td className="px-4 py-3">
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: PLATFORM_COLORS[post.platform] + '22', color: PLATFORM_COLORS[post.platform] }}>{post.platform}</span>
                    </td>
                    ...
                    <td className="px-4 py-3"><span className={`font-semibold ${post.er > 5 ? 'text-[#62d070]' : post.er > 2 ? 'text-[#fad74f]' : 'text-gray-400'}`}>{post.er}%</span></td>
                    <td className="px-4 py-3"><span className="text-xs px-2 py-0.5 rounded-full bg-[#1e2433] text-gray-300">{post.topic}</span></td>
                  </tr>
                  {expanded === i && (
